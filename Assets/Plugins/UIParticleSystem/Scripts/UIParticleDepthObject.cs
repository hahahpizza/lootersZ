...
[ExecuteInEditMode]
public class UIParticleDepthObject : MonoBehaviour
{
    ...
    {
        if(needEditorRefresh)//need to be done because theres no editor mode coroutine to refresh this
        {
            needEditorRefresh = false;
            RefreshRenderer(true, true);
        }
    }

	protected void OnDrawGizmosSelected()
	{
		CheckForRefeshRenderer();
	}
